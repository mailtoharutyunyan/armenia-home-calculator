...
export const COEFF = {
  ...
  // Опалубка: площадь контакта с бетоном на 1 м³.
  // Проверка на базовом доме: перекрытия 53 м³/0.18 = ~296 м² низа, колонны
  // 16 × 4 грани × 6 м = 154 м², ригели 162 пог.м ≈ 194 м², лента 81 м × 2
  // борта × 0.8 = 130 м². Итого ~780 м² на 122 м³ = 6.4. Берём 6.5.
  formworkPerM3: 6.5,
  // Разуклонка плоской кровли: уклонообразующий слой (керамзитобетон/пенобетон).
  // Без него плоская кровля в РА не делается — снег и перепады температур.
  roofSlopeLayer: 0.1, // m, средняя толщина слоя
